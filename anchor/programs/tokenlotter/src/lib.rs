...
use anchor_spl::{associated_token::AssociatedToken, token_interface::{Mint,mint_to,MintTo,transfer_checked,TransferChecked, TokenAccount, TokenInterface},metadata::{Metadata,SignMetadata, sign_metadata}};

use anchor_spl::metadata::{create_metadata_accounts_v3,MetadataAccount,CreateMetadataAccountsV3,create_master_edition_v3,CreateMasterEditionV3,set_and_verify_sized_collection_item,SetAndVerifySizedCollectionItem};
...
,        )?;
    ...
    pub fn buy_ticket(ctx:Context<BuyTicket>)->Result<()>{
        ...
        if clock.slot < ctx.accounts.token_lottery.start_time || clock.slot > ctx.accounts.token_lottery.lottery_end{
        ...
        mint_to(
            CpiContext::new_with_signer(
                ...
                MintTo { mint:ctx.accounts.ticket_mint.to_account_info(), to: ctx.accounts.destination.to_account_info(), authority: ctx.accounts.collection_mint.to_account_info() },
                &signer_seeds
            ),1,)?;

            create_metadata_accounts_v3(
                CpiContext::new_with_signer(ctx.accounts.token_metadata_program.to_account_info(),CreateMetadataAccountsV3{
                    metadata:ctx.accounts.ticket_metada.to_account_info(),
                    mint:ctx.accounts.ticket_mint.to_account_info(),
                    mint_authority:ctx.accounts.collection_mint.to_account_info(),
                    payer:ctx.accounts.payer.to_account_info(),
                    update_authority:ctx.accounts.collection_mint.to_account_info(),
...
pub struct BuyTicket<'info>{
    ...
    #[account(init,payer=payer,seeds = [token_lottery.ticket_num.to_le_bytes().as_ref()],bump,mint::decimals=0,mint::authority=collection_mint,mint::freeze_authority=collection_mint)]
    pub ticket_mint:InterfaceAccount<'info,Mint>,
    ...
    /// CHECK: this account is checked by the metadata smart contract
    pub ticket_metada : UncheckedAccount<'info>,
